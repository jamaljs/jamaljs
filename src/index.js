...
Jml.initialize = function(config = { customTags: [], autoId: true, originalNames: false }) {
      ...
      tags.push(
        (_ => {
          ...
          const parts = customTag
            .split("-")
            .map(x => x.replace(/^./, fc => fc.toUpperCase()));
          return parts.join("");
        })(customTag)
      )
    );
  ...
  tags.forEach(tag => {
    ...
    window[tagIdentifier] = (
      attributes,
      content,
      config = { debug: false, inspect: false, loading: null }
    ) => {
      const { attrs, children } = this.processParameters({
        attributes,
        content
      });
      ...
      Object.keys(attrs).map(key => {
        ...
        if (typeof attrs[key] === "function") {
          const eventHandlerId = `jmlEventHandler_${key}${
            element.attributes["_id"].value
          }`;
          const onEventKey = key
            .replace(/^./, _ => "")
            .replace("on", "")
            .toLowerCase();
            ...
            if (Array.isArray(context)) {
              context.forEach(promiseObject => {
                parentElement.appendChild(
                  typeof promiseObject === "string"
                    ? document.createTextNode(promiseObject)
                    : promiseObject
                );
              });
            } else {
